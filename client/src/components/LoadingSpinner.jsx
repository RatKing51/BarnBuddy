...
export function LoadingSpinner({ label = "Loading..." }) {
  return (
    <div className="inline-flex items-center gap-2 text-sm text-gray-300" role="status" aria-live="polite">
      <span aria-hidden="true" className="h-4 w-4 animate-spin rounded-full border-2 border-blue-400 border-t-transparent" />
      <span>{label}</span>
    </div>
  );
}
...
export function PageLoadingBar({ active }) {
  ...
  return (
    <div aria-hidden="true" className="fixed left-0 right-0 top-0 z-50 h-1 overflow-hidden bg-blue-950">
